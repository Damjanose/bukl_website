  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
      ...
      <div v-if="currentStep === 1">
        <h3 class="text-lg font-semibold mb-4">Number of Websites</h3>
        <input
...
const canProceed = computed(() => {
  if (currentStep.value === 1) {
    return numberOfWebsites.value > 0;
  } else if (currentStep.value === 2) {
    return domains.value.length === numberOfWebsites.value && domains.value.every(domain => domain.trim() !== '');
...
const handleNextStep = () => {
  if (currentStep.value === 1) {
    domains.value = Array.from({ length: numberOfWebsites.value }, () => '');
  }
